...
class ParticleSystem {
    constructor(params) {
        ...
        const uniforms = {
            diffuseTexture: {
                value: new THREE.TextureLoader().load('./src/particle-system/resources/' + this._params.texture + '.png')
            },
            pointMultiplier: {
    ...
    _AddParticles(timeElapsed) {
        ...
        // console.log(n);
        this._params.radio = (this._params.radio)?this._params.radio:2
        for (let i = 0; i < n; i++) {
            const life = (Math.random() * 0.75 + 0.25) * this._params.life;
            this._particles.push({
                position: new THREE.Vector3(
                    (Math.random() * this._params.radio - this._params.radio/2) * 1.0,
                    (Math.random() * 2 - 1) * 1.0,
                    (Math.random() * this._params.radio - this._params.radio/2) * 1.0),
